...
# ── Gameplay frame set ───────────────────────────────────────────────────────
# Use the is_gameplay column stamped by players_ball_detections.py.
# Legacy fallback included for compatibility with older runs.
...
print(f"Loaded {len(gameplay_frames)} gameplay frames.")
...
for frame_id, group in pred_df.groupby("frame_id"):
    ...
    shots = (
        group.set_index("player_id")["shot_type"]
        .fillna("none")
        .astype(str)
        .str.lower()
        .to_dict()
...
TRAIL_LENGTH = max(1, int(fps * TRAIL_SECONDS))
...
trails = {1: deque(maxlen=TRAIL_LENGTH), 2: deque(maxlen=TRAIL_LENGTH)}
